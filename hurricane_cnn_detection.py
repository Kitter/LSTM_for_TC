...
# Read Data
path="/export/kim79/h2/finished_npy/7/hurricane_crop_hr/"
batch_xs_in=np.load(path+"hurricane_input.npy")
batch_ys_in=np.load(path+"hurricane_label.npy")
...
#START
W_conv1=[0,0,0];W_conv2=[0,0,0];b_conv1=[0,0,0];b_conv2=[0,0,0];h_conv1=[0,0,0];h_pool1=[0,0,0];h_conv2=[0,0,0];h_pool2=[0,0,0];pre=[0,0,0];
W_conv1_2=[0,0,0];W_conv2_2=[0,0,0];b_conv1_2=[0,0,0];b_conv2_2=[0,0,0];h_conv1_2=[0,0,0];h_conv2_2=[0,0,0];
label_size=2;
...
for i in range(channels):
    #(1) First Convolutional Layer
     ...
     x_image=tf.reshape(x[i],[-1,40,40,1])
    ...
    #(1-1) First-2 Convolutional Layer
     W_conv1_2[i] = weight_variable([5, 5, 32, 32])
     b_conv1_2[i] = bias_variable([32])
     h_conv1_2[i] = tf.nn.relu(conv2d(h_conv1[i], W_conv1_2[i]) + b_conv1_2[i])
     h_pool1[i] = max_pool_2x2(h_conv1_2[i])
    ...
    #(2-2) Second-2 Convolutional Layer
     W_conv2_2[i] = weight_variable([5, 5, 64, 64])
     b_conv2_2[i] = bias_variable([64])
     h_conv2_2[i] = tf.nn.relu(conv2d(h_conv2[i], W_conv2_2[i]) + b_conv2_2[i])
     h_pool2[i] = max_pool_2x2(h_conv2_2[i])

     
#(3) Densely Connected Layer
h_pool2_concat=tf.concat([h_pool2[0],h_pool2[1],h_pool2[2]],3)
...
#(5) Readout Layer
W_fc2 = weight_variable([64, label_size]) 
...
for i_conv in range(20000):
    ...
    if i_conv%100 == 0:
        train_accuracy = sess.run(accuracy,feed_dict={x[0]:batch_x[0], x[1]:batch_x[1], x[2]:batch_x[2],  y_: batch_y, keep_prob: 1.0})
        print(" %dth Training Step, training accuracy %g"%(i_conv, train_accuracy))
        fo.write(" %dth Training Step, training accuracy %g"%(i_conv, train_accuracy))

    #Actual Training
    sess.run(train_step,feed_dict={x[0]:batch_x[0], x[1]:batch_x[1],x[2]:batch_x[2], y_: batch_y, keep_prob: 0.5})
    #(8)Print Acuuracy
    if i_conv%20 == 0:
        print("\t %d : test accuracy %g"%(i_conv,sess.run(accuracy,feed_dict={ 
             x[0]:test_data[:,:,:,0], x[1]:test_data[:,:,:,1],x[2]:test_data[:,:,:,2],  y_:test_label, keep_prob: 1.0})))
        fo.write("\t %d : test accuracy %g \n"%(i_conv,sess.run(accuracy,feed_dict={
             x[0]:test_data[:,:,:,0], x[1]:test_data[:,:,:,1],x[2]:test_data[:,:,:,2], y_:test_label, keep_prob: 1.0})))
pre=sess.run(prediction,feed_dict={x[0]:test_data[:,:,:,0], x[1]:test_data[:,:,:,1],x[2]:test_data[:,:,:,2], y_:test_label, keep_prob: 1.0})
ans=batch_y;
